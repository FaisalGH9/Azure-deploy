...
class ProcessingEngine:
    ...
    async def summarize_video(self, video_id: str, length: str = "medium") -> str:
        ...
        # Split content into ~4000-token-safe chunks (about 16k chars)
        chunk_size = 16000
        ...
        for idx, chunk in enumerate(content_chunks):
            print(f"🧩 Summarizing chunk {idx + 1}/{len(content_chunks)}")
            part_summary = await self.llm_provider.summarize(chunk, length)
            partial_summaries.append(part_summary)

        combined_summary = "\n\n".join(partial_summaries)

        # Step 2: Final summary pass (only if more than 1 chunk)
        if len(partial_summaries) > 1:
            print("📚 Refining overall summary...")
            combined_summary = await self.llm_provider.summarize(combined_summary, length)

        # Cache and return
        self.cache_manager.cache_response(video_id, cache_key, combined_summary)
